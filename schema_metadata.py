...
Single source of truth for MIMIC-IV table/column metadata.

This is intentionally NOT a vector store and NOT tied to Groq/OpenAI/Pinecone —
it's a plain Python data structure. Two consumers read from it:

  1. build_index.py        -> embeds these descriptions into Pinecone (run once / on schema change)
  2. tools.py (get_schema) -> looks up exact column lists for tables the LLM asks for

Adding a new MIMIC table = add one TableSchema entry here. Nothing else changes.

GRANULARITY STRATEGY (hybrid, per user's decision):
  - Narrow tables (patients, admissions, diagnoses_icd, procedures_icd, etc.):
        embed ONE document for the whole table.
  - Wide "event" tables (chartevents, labevents, d_labitems, d_items, prescriptions):
        embed ONE table-level document AND one document PER COLUMN (or per logical
        column group), because the clinically meaningful unit in these tables is
        often a single column/value (e.g. "creatinine", "heart rate"), not the table.

  The `wide` flag on TableSchema controls this — see build_index.py for how it's used.
"""
...
class Column:
    name: str
    dtype: str
    description: str = ""  # used for column-level embedding docs on wide tables


@dataclass
class TableSchema:
    name: str
    module: str  # "hosp" or "icu" — mirrors MIMIC-IV's actual module split
    description: str  # used for the table-level embedding doc
    columns: list[Column]
    wide: bool = False  # True => also generate per-column embedding docs
    join_hints: str = ""  # plain-English join keys, folded into the embedding doc


# ---------------------------------------------------------------------------
# NARROW TABLES — table-level embedding only
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# WIDE TABLES — table-level + per-column embedding docs
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Registry — add new tables here and nowhere else
# ---------------------------------------------------------------------------
